...
//                  }
                    ...
                    String sqlTemp = sql;
                    List<String> a = getValue();
                    List<String> c = getType();
                    int limit = 0;
                    ...
                    for (int i = 0; i < limit; i++)
                    {
                        ...
                        if(oo == null)
                        {
                            sqlTemp = sqlTemp.replaceFirst("\\?","null");
                        }
                        else 
                        {
                            if("java.lang.String".equals(oo.getClass().getName()) == true)
                            {
                                sqlTemp = sqlTemp.replaceFirst("\\?","'" + String.valueOf(a.get(i)).replace("'", "''")+ "'");
                            }
                            else
                            {
                                sqlTemp = sqlTemp.replaceFirst("\\?",String.valueOf(a.get(i)));
                            }
                        }
                    }
                  //log.debug("{pstm-" + id + "} Executing Statement: " + removeBreakingWhitespace(sql));
                  //RYS
                  //log.debug("{pstm-" + id + "} Executing Statement: " + removeBreakingWhitespace(sqlTemp));
                    log.debug("{pstm-" + id + "} Executing Statement : " + sqlTemp);
//===================================================================                    
                    log.debug("{pstm-" + id + "} Parameters: " + getValueString());
                    log.error("{pstm-" + id + "} Types: " + getTypeString());
                    log.warn( "{pstm-" + id + "} Types: " + getTypeString());
                }
